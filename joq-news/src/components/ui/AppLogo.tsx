/**
 * Theme-aware JOQ logo component.
 * Renders the SVG logo with automatic color switching:
 *   - Dark mode → white logo
 *   - Light mode → dark logo
 * Accepts optional `color` override and responsive `width`.
 */
...
/** Original viewBox: 0 0 300 120 → aspect ratio 2.5:1 */
const VIEWBOX = '0 0 300 120';
const ASPECT = 300 / 120;

// Paths extracted from DarkLogoJoq.svg / WhiteLogoJoq.svg (identical geometry)
const LOGO_PATHS = [
  // "Q" letter with decorative tail
...
interface AppLogoProps {
  /** Logo width in points — height is auto-calculated (2.5:1 ratio) */
  width?: number;
  /** Override fill color — otherwise auto-selects based on theme */
  color?: string;
}

export function AppLogo({ width = 120, color }: AppLogoProps) {
  const { dark, colors } = useTheme();

  // Auto-select: white on dark backgrounds, near-black on light
  const fill = color ?? (dark ? '#FFFFFF' : colors.text);
